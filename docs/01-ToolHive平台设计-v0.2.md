...
**与 embedding 的本质区别**：embedding 把 query 与文档**分别**编码再算余弦（两者从不"见面"）；cross-encoder 把两者**拼在一起**送进模型，能建模词级交互。
...
**这就是精排的全部价值**：embedding 分不开这两者（都含"客户/查询"），cross-encoder 能注意到"投诉 ↔ 工单"的对应与"档案 ↔ 投诉"的不对应。

**为什么不需要业务知识**：它只判断**文本相关性**。业务映射（"投诉"属于哪个域）由 `scope` / `taxonomy` 承担，发生在精排**之前**；业务推理属于上层系统。让 reranker 做业务推理，它就变成生成式决策，约束被破坏。

**代价与约束**：cross-encoder 必须对每个候选各算一次前向（O(N)），候选数量直接决定延迟：
...
这些在"投诉 vs 档案"这类语义细微差别上无能为力——那正是 cross-encoder 的价值。
...
上游 Token（明文）
  └─ 用 DEK 加密        → ciphertext 存入 credential 表
       └─ DEK 再用 KEK 加密后一并存储
            └─ KEK 放在库外：环境变量 / 配置中心 / KMS，永不入库
```

`kek_id` 字段记录"这条密文由哪把 KEK 包装"。将来轮换 KEK 时，老密文用老 KEK 解、新写入用新 KEK 加，可平滑过渡，不必一次性全量重加密。

**M0 的做法**：KEK 由单个 32 字节环境变量注入；每条凭据使用独立随机 DEK + AES-256-GCM。KMS 集成留到 M1。
...
| 精排（M1，评测触发） | **cross-encoder reranker**（判别式，非 LLM）；优先复用现有模型服务商的 rerank 接口，否则独立部署 | 见 §6.2；**"CPU + 100 候选 + base 模型"需实测延迟** |
...
| **精排（rerank）** | **不做**，用 RRF 融合输出 | M1 |
| **多租户** | **不做**，`tenant_id` 字段预留 | — |
| **KEK** | **环境变量注入** | KMS 留到 M1，见 §10.1（Q6） |
...
| D2 | 凭据存储 | **密文入库**（信封加密），并预留 `external_ref` 字段以支持将来切到外部 Vault | §10.1 |
...
| D8 | M0 范围与实现细节 | **共 13 项，见 §16.2 的「M0 范围决策」表**（管理面形态、认证方式、MCP 前端、确认令牌、关键词检索、富化、精排、多租户、KEK、审批权限、免审策略、审计保留、工具服务 SDK） | §16.2 |
...
| R7 | KMS 集成 | M1 | M0 用环境变量注入 KEK |
| R8 | 中文分词 / BM25 升级 | 评测触发，可能永不引入 | M0 用 `pg_trgm` 绕开，见 §6.2 |
| R9 | 精排（reranker）是否引入、选型与部署形态 | **评测触发**（诊断规则见 §6.2） | 需实测 CPU/GPU 延迟；先查现有服务商有无 rerank 接口 |
